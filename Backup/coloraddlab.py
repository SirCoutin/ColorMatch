...
if os.path.exists("stamp_colors_lab.json"):
  with open("stamp_colors.json", "r") as colors:
            stamp_colors = json.load(colors)
else:
  print("Creating new database")
  with open("stamp_colors_lab.json", "w") as colors:
            json.dump(stamp_colors, colors)

def quantize_lab(lab, step=1):
    return (
        round(lab.lab_l / step) * step,
        round(lab.lab_a / step) * step,
        round(lab.lab_b / step) * step,
    )
...
def add_color_to_michel(image_path, name, stamp_name):
    ...
    for pixel in pixels:
        ...
        quantized = quantize_lab(lab)
        lab_pixel_array.append(quantized)
    ...
    print(f"Added '{name}' with Lab value {most_common_match} successfully!")
while True:
